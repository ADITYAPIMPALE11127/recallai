...
export const CoverImageModal = () => {
  ...
  const onChange = async (file?: File) => {
    if (file) {
      ...
      // Convex removed: persist to backend via Django (if available).
      // For now we close after successful upload.
      // TODO: wire this to a Django API endpoint for updating document cover image.
      onClose();
    }
  };
  ...
  const onSelectColor = async (color: string) => {
    ...
    // Convex removed. Django backend currently only supports Note fields
    // (title/content/created_at/updated_at), so there is no coverImage column.
    // Keep the UI behavior consistent by closing the modal.
    onClose();
  };
